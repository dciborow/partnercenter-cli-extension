...
def load_arguments(commands_loader, _):
    ...
    with commands_loader.argument_context("partnercenter marketplace offer setup update") as c:
        ...
        c.argument(
            Arg.reseller, arg_type=get_three_state_flag(), options_list=["--reseller"], help=arg_help[Arg.reseller]
        )
        c.argument(
            Arg.test_drive,
            arg_type=get_three_state_flag(),
            options_list=["--test-drive"],
            help=arg_help[Arg.test_drive],
            validator=validate_test_drive,
        )
        c.argument(
            Arg.sell_through_microsoft,
            arg_type=get_three_state_flag(),
            options_list=["--sell-through-microsoft"],
            help=arg_help[Arg.sell_through_microsoft],
        )
