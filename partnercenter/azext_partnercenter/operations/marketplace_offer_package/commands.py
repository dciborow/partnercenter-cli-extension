...
def load_command_table(commands_loader, _):
    custom_command_type = CliCommandType(
        operations_tmpl="azext_partnercenter.operations.marketplace_offer_package.custom#{}", client_factory=cf_offers
    )

    with commands_loader.command_group(
        "partnercenter marketplace offer package", custom_command_type=custom_command_type, is_preview=True
    ) as g:
