...
def load_command_table(commands_loader, _):
    custom_command_type = CliCommandType(
        operations_tmpl="azext_partnercenter.operations.marketplace_offer_listing_media.custom#{}",
        client_factory=cf_listing_media,
    )

    with commands_loader.command_group(
        "partnercenter marketplace offer listing media", custom_command_type, is_preview=True
    ) as g:
        ...
        g.custom_command(
            "delete", "delete_media", table_transformer=None, confirmation=True, custom_command_type=custom_command_type
        )
