...
def load_command_table(commands_loader, _):
    command_type = CliCommandType(
        operations_tmpl="azext_partnercenter.clients#PlanListingClient.{}", client_factory=cf_plan_listing
    )
    custom_command_type = CliCommandType(
        operations_tmpl="azext_partnercenter.operations.marketplace_offer_plan_listing.custom#{}",
        client_factory=cf_plan_listing,
    )

    with commands_loader.command_group(
        "partnercenter marketplace offer plan listing",
