...
class PartnerCenterMarketplaceOfferSetupScenarioTest(PartnerCenterScenarioTest):
    ...
    def test_marketplace_offer_setup_test_drive_on_vm_type(self):
        self.cmd(
            "partnercenter marketplace offer setup update --offer-id {offer_id} --test-drive {updated_test_drive}",
    ...
    def test_marketplace_offer_setup(self):
        ...
        self.cmd(
            "partnercenter marketplace offer setup update --offer-id {offer_id} --sell-through-microsoft {updated_sell_through_microsoft} --trial-uri {updated_trial_uri}",
            checks=[self.check("sellThroughMicrosoft", True), self.check("trialUri", "{updated_trial_uri}")],
        )

        self.cmd(
            "partnercenter marketplace offer setup show --id {offer_id}",
            checks=[
                self.check("sellThroughMicrosoft", True),
