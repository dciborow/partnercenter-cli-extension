...
class PartnerCenterMarketplaceOfferListingContactScenarioTest(PartnerCenterScenarioTest):
    ...
    def test_marketplace_offer_listing_contact(self):
        self.cmd(
            "partnercenter marketplace offer listing contact add --offer-id {offer_id} --type {contact_type} --email {contact_email} --name '{contact_name}' --phone {contact_phone} --uri {contact_uri}",
            checks=[
        ...
        self.cmd(
            "partnercenter marketplace offer listing contact delete --yes --offer-id {offer_id} --type {contact_type} --email {contact_email} --name '{contact_name}' --phone {contact_phone} --uri {contact_uri}"
        )

        output = self.cmd(
            "az partnercenter marketplace offer listing contact list --offer-id {offer_id}"
        ).get_output_in_json()
        assert len(output) == 0
