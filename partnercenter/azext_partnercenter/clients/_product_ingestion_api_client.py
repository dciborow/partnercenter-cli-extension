...
class ProductIngestionApiClient:
    ...
    def update_container_plan_technical_configuration_for_bundle(
        self, offer_durable_id, plan_durable_id, properties=ContainerCnabPlanTechnicalConfigurationProperties | None
    ):
        ...
        configuration = ContainerPlanTechnicalConfiguration(
            id=durable_id.__root__, product=product_id.__root__, plan=plan_id.__root__
        )
        ...
        resource[
            "$schema"
        ] = "https://product-ingestion.azureedge.net/schema/container-plan-technical-configuration/2022-03-01-preview3"
    ...
    def publish_submission(self, target, offer_durable_id, submission_id=None):
        ...
        product_id = DurableId(__root__="product/" + offer_durable_id)
        durable_id = (
            DurableId(__root__=f"submission/{offer_durable_id}/{submission_id}") if submission_id is not None else None
        )
