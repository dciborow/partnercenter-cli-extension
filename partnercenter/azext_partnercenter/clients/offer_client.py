...
from azext_partnercenter.models import (
    Listing,
    ListingContact,
    ListingUri,
    Offer,
...
class OfferClient(BaseClient):
    ...
    def list(self):
        results = get_combined_paged_results(
            lambda skip_token: self._sdk.product_client.products_get(self._get_access_token(), skip_token=skip_token)
        )
    ...
    def create(self, offer_external_id, offer_alias, resource_type):
        ...
        product = MicrosoftIngestionApiModelsProductsAzureProduct(
        ...
        product = self._sdk.product_client.products_post(
            authorization=authorization, microsoft_ingestion_api_models_products_azure_product=product
        )
    ...
    def get_setup(self, offer_external_id):
        ...
        result = self._sdk.product_client.products_product_id_setup_get(
            offer._resource.durable_id, self._get_access_token()
        )
    ...
    def update_setup(self, parameters: OfferSetup):
        ...
        channel_state = MicrosoftIngestionApiModelsCommonTypeValuePair(
            type="Reseller", value="Enabled" if parameters.reseller else "Disabled"
        )
    ...
    def get_listing(self, offer_external_id):
        ...
        branch_listings = get_combined_paged_results(
            lambda: self._sdk.branches_client.products_product_id_branches_get_by_module_modulemodule_get(
        ...
        return Listing(
            ...
            if hasattr(listing, "getting_started_instructions")
            else "",
