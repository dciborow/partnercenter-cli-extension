...
class PlanTechnicalConfigurationClient(BaseClient):
    ...
    def get(self, offer_external_id, plan_external_id):
        ...
        if variant_package_branch.product.resource_type == "AzureContainer":
            technical_configuration = self._graph_api_client.get_container_plan_technical_configuration(
                offer_durable_id, plan_durable_id, sell_through_microsoft
            )
        else:
            technical_configuration = self._get_plan_technical_configuration(
                variant_package_branch.product.id, variant_package_branch.variant_id
            )
    ...
    def add_bundle(
        self, offer_external_id, plan_external_id, properties=ContainerCnabPlanTechnicalConfigurationProperties | None
    ):
        variant_package_branch = self._get_variant_package_branch(offer_external_id, plan_external_id)
        offer_durable_id = variant_package_branch.product.id
        plan_durable_id = variant_package_branch.variant_id
        ...
        if len(status.errors) > 0:
            return {"errors": status.errors, "result": status.job_result}
        else:
            return {"message": status.job_status.value, "result": status.job_result}
    ...
    def _get_variant_package_branch(self, offer_external_id, plan_external_id):
        ...
        variant_package_branch = next(
            (b for b in branches.value if hasattr(b, "variant_id") and b.variant_id == variant["id"]), None
        )
